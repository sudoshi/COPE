...
**Status:** Complete — live at https://cope.acumenus.net

---

## 1. Objective

Deploy COPE to a production environment at `https://cope.acumenus.net` served through the existing Apache2 installation, with Let's Encrypt SSL, systemd-managed services, and automated near-real-time redeployment — all running **concurrently** alongside the development servers on localhost.
...
Internet
  │
  ▼
Apache2 (:443 SSL)  ─── cope.acumenus.net
  │
...
| Apache HTTPS  | —        | 443       | Serves cope.acumenus.net    |
...
An A record for `cope.acumenus.net` was pointed to the server's public IP (`50.32.71.185`). Verified via `dig`:

```
cope.acumenus.net.   300   IN   A   50.32.71.185
```
...
| `CORS_ORIGIN`  | `http://localhost:5173`  | `https://cope.acumenus.net`        |
...
**`/etc/apache2/sites-available/cope.acumenus.net.conf`** — HTTP redirect:

```apache
<VirtualHost *:80>
    ServerName cope.acumenus.net
    RewriteEngine On
...
**`/etc/apache2/sites-available/cope.acumenus.net-le-ssl.conf`** — HTTPS vhost:
...
<VirtualHost *:443>
    ...
    # SSL
    SSLEngine on
    SSLCertificateFile /etc/letsencrypt/live/cope.acumenus.net/fullchain.pem
    SSLCertificateKeyFile /etc/letsencrypt/live/cope.acumenus.net/privkey.pem
    Include /etc/letsencrypt/options-ssl-apache.conf

    # Static web dashboard
    DocumentRoot /home/smudoshi/Github/COPE/apps/web/dist
    <Directory /home/smudoshi/Github/COPE/apps/web/dist>
    ...
    # Logging
    ErrorLog ${APACHE_LOG_DIR}/cope-error.log
    CustomLog ${APACHE_LOG_DIR}/cope-access.log combined
</VirtualHost>
</IfModule>
```
...
```bash
sudo certbot --apache -d cope.acumenus.net --non-interactive --agree-tos
```

**Issue encountered:** The existing `demo.acumenus.net` config had `ServerAlias *`, acting as a catch-all for all hostnames. Certbot deployed the cope cert into the demo SSL config instead of creating a new one. This was resolved by:

1. Removing `ServerAlias *` from both demo vhost configs
2. Creating a dedicated `cope.acumenus.net-le-ssl.conf` with the full proxy/static config
...
**`cope-api.service`** — Fastify API server:

```ini
[Service]
User=smudoshi
WorkingDirectory=/home/smudoshi/Github/COPE/apps/api
EnvironmentFile=/home/smudoshi/Github/COPE/.env.production
ExecStart=/usr/bin/node dist/server.js
Restart=on-failure
RestartSec=5
NoNewPrivileges=true
ProtectSystem=strict
ReadWritePaths=/home/smudoshi/Github/COPE
```

**`cope-worker.service`** — BullMQ background worker:

```ini
[Service]
# Same as API but runs worker.js
ExecStart=/usr/bin/node dist/worker.js
After=cope-api.service    # Starts after API
```

**`cope-auto-deploy.service`** — Automated rebuild daemon:

```ini
[Service]
User=root                    # Needs root for systemctl restart
ExecStart=/home/smudoshi/Github/COPE/scripts/auto-deploy.sh
...
```bash
sudo systemctl enable cope-api cope-worker cope-auto-deploy
```
...
- `ReadWritePaths=/home/smudoshi/Github/COPE` — only the repo directory is writable
...
│     c. If build succeeds:                           │
│        - `systemctl restart cope-api worker`     │
...
- **Lock file:** `/tmp/.cope-deploy.lock` prevents overlapping builds if one takes longer than 60 seconds
...
journalctl -u cope-auto-deploy -f

# Check last few deploys
journalctl -u cope-auto-deploy --since "1 hour ago"

# Temporarily stop auto-deploy
sudo systemctl stop cope-auto-deploy

# Permanently disable
sudo systemctl disable cope-auto-deploy
```
...
2. Restarts `cope-api` and `cope-worker` via systemd
...
| `/etc/apache2/sites-available/cope.acumenus.net.conf` | HTTP → HTTPS redirect |
| `/etc/apache2/sites-available/cope.acumenus.net-le-ssl.conf` | Full HTTPS vhost with proxy + static serving |
| `/etc/systemd/system/cope-api.service` | Production API systemd unit |
| `/etc/systemd/system/cope-worker.service` | Production worker systemd unit |
| `/etc/systemd/system/cope-auto-deploy.service` | Auto-deploy daemon systemd unit |
...
| Test | Result |
|------|--------|
| `curl https://cope.acumenus.net/health` | `{"status":"ok","db":"connected"}` |
| `curl https://cope.acumenus.net/` | 200 — HTML loads |
| `systemctl is-active cope-api` | `active` |
| `systemctl is-active cope-worker` | `active` |
| `systemctl is-active cope-auto-deploy` | `active` |
...
```bash
journalctl -u cope-api -n 50           # API logs
journalctl -u cope-worker -n 50        # Worker logs
journalctl -u cope-auto-deploy -n 50   # Auto-deploy logs
```

### Apache logs

```bash
sudo tail -50 /var/log/apache2/cope-error.log
sudo tail -50 /var/log/apache2/cope-access.log
```
...
| 502 Bad Gateway on `/api/` | API not running on port 3080 | `sudo systemctl restart cope-api` |
| SSL errors | Cert not found or expired | `sudo certbot renew` |
| Auto-deploy not triggering | Timestamp file newer than changes | `rm /tmp/.cope-last-deploy-hash` |
| Build stuck / lock held | Stale lock from killed build | `rm /tmp/.cope-deploy.lock` |
...
- **Separate database:** Production currently shares `copedemo` with dev. For a true production environment, a dedicated database with its own credentials should be provisioned.
...
- **Health check monitoring:** Consider adding an external uptime monitor (e.g., UptimeRobot) for `https://cope.acumenus.net/health`.
