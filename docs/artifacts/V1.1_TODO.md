...
- [ ] Confirm demo seed works: login as `dr.kim@copedemo.com` via API
...
- [x] Implement `sendInviteEmail()` using `resend` client
  - [x] Subject line: `{orgName} — Your clinician has invited you to COPE`
  - [x] Deep link CTA: `cope://invite?token={token}`
...
- [x] Add `"scheme": "cope"` at the `expo` root level (if not present)
- [x] Add Android `intentFilters` for `cope://invite`
- [x] Add iOS URL scheme for `cope`
...
- [x] Add `useEffect` for deep link handling:
  ...
  - [x] If URL matches `cope://invite?token=*` → `router.replace('/onboarding?token=...')`
...
- [x] Add `INTAKE_COMPLETE: 'cope_intake_complete'` to `KEYS` object
...
- [x] **Screen 2 — Required Consents**
  ...
  - [x] "Both are required to use COPE" caption
...
- [ ] Android deep link: `adb shell am start ... cope://invite?token=XXX` → opens registration tab with token ✓
...
- [x] Register queue `cope-ai-insights` in BullMQ
...
- [ ] Create `scripts/backup.sh`
  - [ ] `pg_dump | gzip | openssl enc -aes-256-cbc -pbkdf2` → `/backups/cope-YYYYMMDD.sql.gz.enc`
