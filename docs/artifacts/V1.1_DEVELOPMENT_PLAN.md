# COPE — Version 1.1 Development Plan
...
**Invite email design:**
- Subject: `{orgName} — Your clinician has invited you to COPE`
- Deep link CTA button: `cope://invite?token={token}`
...
- "Both are required to use COPE" caption.
...
- [ ] Android deep link: `adb shell am start -a android.intent.action.VIEW -d "cope://invite?token=XXX"` → onboarding opens with token pre-filled
...
**New dependency:** `expo-speech` (already in Expo SDK) for synthesis; `@react-native-voice/voice` for recognition. However, `@react-native-voice/voice` is a bare workflow dependency. Since COPE uses Expo managed workflow, use the Web Speech API via `expo-web-browser` or switch to **Whisper API** (Anthropic / OpenAI) for transcription of recorded audio clips.
...
- Permission dialog explains: "COPE reads step count, heart rate, and sleep data to enrich your daily report. This data stays on your device and server — it is never sold."
...
```
EXPO_PUBLIC_API_BASE          = https://app.cope.health
...
**Duration:** Weeks 4–7
**Goal:** Produce a standards-compliant FHIR R4 REST API layer that allows hospital EHR systems to query COPE patient data, and a CDA handover document for paper/email discharge summaries.

### 8.1 FHIR R4 Resource Mapping

| FHIR Resource | COPE Source | Notes |
|---------------|----------------|-------|
...
Pure mapping functions, no DB calls. Takes a COPE domain object and returns the FHIR equivalent.
...
jobs:
  ...
  test-api:
    ...
    services:
      postgres:
        image: postgres:17
        env: { POSTGRES_DB: cope_test, POSTGRES_PASSWORD: test }
...
pg_dump "$DATABASE_URL" | gzip | openssl enc -aes-256-cbc -pbkdf2 \
  -pass "env:BACKUP_ENCRYPTION_KEY" > "/backups/cope-$(date +%Y%m%d).sql.gz.enc"
# Retain 30 days; delete older
find /backups -name "cope-*.sql.gz.enc" -mtime +30 -delete
```
...
COPE v1.1 does not ship i18n but lays the groundwork:
...
COPE is likely a **Software as a Medical Device (SaMD)** under 21 CFR Part 820. The current intended use (monitoring patient-reported outcomes and alerting clinicians) likely places it in:
...
*End of COPE Version 1.1 Development Plan*
